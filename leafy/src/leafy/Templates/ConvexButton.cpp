...
template <> /// Explicit Button sf::ConvexShape Button() Instantiation
Button<sf::ConvexShape>::Button(const sf::ConvexShape& shape)
    : m_shape(nullptr)
    , m_clicked(false)
{
    m_shape = new sf::ConvexShape(shape);
}
template <> /// Explicit Button sf::ConvexShape ~Button() Instantiation
Button<sf::ConvexShape>::~Button()
{
    if (m_shape)
        delete m_shape;
}
template <>
bool Button<sf::ConvexShape>::clicked() const
{
    return m_clicked;
}
template <>
bool Button<sf::ConvexShape>::contains(sf::Vector2f point) const
{
    return m_shape->getGlobalBounds().contains(point);
}
template <>
void Button<sf::ConvexShape>::mouseOver()
{
    m_shape->setFillColor({m_shape->getFillColor().r, m_shape->getFillColor().g, m_shape->getFillColor().b, 100});
}
template <>
void Button<sf::ConvexShape>::mouseLeave()
{
    m_shape->setFillColor({m_shape->getFillColor().r, m_shape->getFillColor().g, m_shape->getFillColor().b, 255});
}
template <>
void Button<sf::ConvexShape>::handleEvent(sf::RenderWindow &window, sf::Event event)
{
...
template <>
void Button<sf::ConvexShape>::draw(sf::RenderTarget& target, sf::RenderStates states) const
{
